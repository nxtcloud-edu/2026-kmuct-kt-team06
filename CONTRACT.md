...
| GET | `/api/review` | `[{id, kind:"low_confidence"\|"grey"\|"rewritten"\|"quote_mismatch", lecture, slug, anchor\|null, text, reason}]` — 사람이 확인할 목록(PRD §4.8). 전부 기존 데이터에서 계산 |
...
```
목 파일: `mock/segments/L3.json` `mock/source.json` `mock/notes/L3.json` `mock/stats.json` `mock/history.json` `mock/qa.json` `mock/qa-nogrounding.json` `mock/review.json`
