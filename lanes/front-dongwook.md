...
- `> 🗣` 인용 블록은 `blockquote` 로 렌더된다 → 칩이 인용 끝에 붙고, 인용 전체를 눌러도 점프하게. `[!youtube]` 콜아웃은 Quartz가 그린다(`.callout[data-callout=youtube]`) — 빨간 테두리 CSS만
- **완료 조건**: `lectures/L3_그래프_탐색` 페이지에서 앵커 10개가 칩으로 보이고, 탐색기로 dfs 로 넘어가도(새로고침 없이) 칩이 된다.
