...
### 4.1b `raw/L{n}/transcript.json` (① 적재의 중간 형식 — 다글로든 Grok STT든 이걸로 맞춘다)
```json
[{"t_start": 312.0, "t_end": 318.4, "text": "BFS는 큐를 씁니다", "agree": 0.96}]
```

`agree`(선택) = 두 전사본의 일치율(PRD §8.9). **0.5 미만 문장은 🗣 인용 금지 + 화면 경고, 0.8 미만은 🔈 표시.** 필드가 없으면 "측정 안 됨".
...
```
목 파일: `mock/segments/L3.json` `mock/source.json` `mock/notes/L3.json` `mock/stats.json` `mock/history.json` `mock/qa.json` `mock/qa-nogrounding.json` `mock/review.json` `mock/quotes.json`
