...
## 엣지케이스 (CONTRACT §5.4)
- 글은 전부 `textContent`. `innerHTML` 금지
- 저장·전송 **연타 방지**(요청 중 버튼 비활성) · 빈 입력 · 8KB 초과는 보내기 전에 막기 · 저장 실패 시 입력 내용 유지
- 답변 대기 중 페이지 이동 → 응답이 와도 패널은 그대로(슬롯은 `#v-root` 안이라 안 지워진다)
- `videos: []` · `anchors: []` · `notes: []` 빈 배열 전부 정상 처리
