...
- 해법: ① 적재에 **전사본 ↔ 슬라이드 텍스트 맞추기**를 넣는다. 슬라이드 순서는 단조 증가라는 제약 + 슬라이드 단어가 전사본에 처음 몰려 나오는 지점 = 경계. 코드(단어 겹침 점수)로 초안 → 애매한 경계만 fast 모델이 판정. 출력은 같은 `segments.json`(`kind:"slide"`, `final_frame:null`, 슬라이드 PNG가 프레임 역할).
- **현장에서 처음 짜기엔 위험하다.** 데모 강의 1~3편은 시작 전에 구간표를 미리 만들고 사람이 5분 검수한다. 발표에서는 "자동 초안 + 검수"라고 그대로 말한다.
- 이것이 사실 이 제품의 핵심 난제이고 심사 ②("에이전트를 쓸 수밖에 없었나")의 가장 정직한 답이다: 규칙만으로는 경계가 안 잡히고, 모델만으로는 초가 틀린다 → 코드 제약 + 모델 판정 + 훅 검증.
