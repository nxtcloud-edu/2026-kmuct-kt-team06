...
- 입력 맞추기: 다글로 전사본 → `raw/L{n}/transcript.json`(CONTRACT §4.1b) 변환기 `pipeline/ingest_daglo.py`. **Grok STT(`pipeline/stt.py`, `XAI_API_KEY`)는 T5 이후 여유 있을 때** 1분 클립으로만
...
- (P2, 강의 2편째를 넣을 때) **Merge 규칙**(PRD §4.10): 기존 개념 페이지의 Current 문단이 바뀌거나 사라지는 쓰기는 `status: draft` 강제 + 검토함 `kind:"merge"`. 추가만이면 자동. 판정은 `write_page_guard.py` 에서 코드로
